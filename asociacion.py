...
#==== La clase A tiene 3 numeros rales ====
...
class B:
    ...
    #==== Metodo sumer todo (internos + externos) ====

    def sumar_todo(self,aa:float,bb:float):
        x:float=self.d+self.e+aa+bb
        return x

#==== ASOCIACION ( Usando objetos independientes) ====
...
class C:
    ...
    def __init__(self,d:float,e:float):
        self.d=d
        self.e=e
        # A se esta instanciando adentro 
...
#==== COMPOSICION (Contiene un objeto detron de otro) ====
...
class D:
    ...
    def sumar_todo(self):
        x:float=self.d+self.e+self.Aa.a+self.Aa.b
        return x
#==== AGREGACION(cosntruye el objeto agregado por fuera) ====
